...
import {
  ArrowLeft, CheckCircle2, XCircle, Mail, FileText, RefreshCw,
  Loader2, Users, CreditCard, FileCheck, MessageSquare,
  History, Send, Plus, Eye
} from "lucide-react";
...
const AdminBookingDetail = () => {
  ...
  useEffect(() => {
    if (!authLoading && user && isAdmin && bookingId) {
      loadBooking();
    }
  ...
  const loadBooking = async () => {
    ...
    try {
      ...
      const [tourRes, dateRes, tariffRes, pickupRes, paymentsRes, auditRes, docsRes] = await Promise.all([
  ...
  const updateBookingStatus = async (newStatus: string) => {
    ...
    try {
      ...
      if (newStatus === "confirmed" && !booking.paid_at) {
  ...
  const handlePaymentReceived = async () => {
    ...
    try {
      ...
      const updateData: any = {
        ...
        payment_reference: paymentForm.reference || booking.payment_reference,
      };
      if (isFullyPaid) {
      ...
      if (paymentForm.sendEmail && isFullyPaid) {
        try {
          ...
          await supabase.from("tour_document_sends").insert({
            booking_id: booking.id,
          ...
          toast({ title: "✅ Zahlung erfasst (Mail fehlgeschlagen)" });
        }
      } else {
  ...
  const regenerateDocuments = async () => {
    ...
    try {
      await supabase.functions.invoke("generate-tour-documents", {
        body: { bookingId: booking.id, documentType: "all" },
      });
  ...
  // Access control
  if (authLoading) {
    return <div className="min-h-screen bg-zinc-950 flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-emerald-400" /></div>;
  }
  if (!user || !isAdmin) {
    return <div className="min-h-screen bg-zinc-950 flex items-center justify-center text-white">Kein Zugriff</div>;
  }
  ...
  const getPaymentStatusColor = (s: string) => {
    switch (s) {
      ...
      case "teil": return "bg-amber-600";
      case "offen": return "bg-red-500";
      default: return "bg-zinc-600";
    }
  };
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <div className="border-b border-zinc-800 bg-zinc-900">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center gap-4 mb-4">
            <Button variant="ghost" size="sm" onClick={() => navigate("/admin/dashboard")} className="text-zinc-400 hover:text-white">
              <ArrowLeft className="w-4 h-4 mr-1" /> Zurück
            </Button>
            <div className="flex-1">
              <h1 className="text-xl font-bold flex items-center gap-3">
                Buchung <span className="text-emerald-400 font-mono">{booking.booking_number}</span>
                <Badge className={getStatusColor(booking.status)}>{booking.status === "confirmed" ? "Bezahlt" : booking.status === "pending" ? "Offen" : booking.status === "cancelled" ? "Storniert" : booking.status}</Badge>
              </h1>
              <p className="text-sm text-zinc-500">{booking.contact_first_name} {booking.contact_last_name} • {tourInfo?.destination} • {dateInfo ? format(new Date(dateInfo.departure_date), "dd.MM.yyyy") : ""}</p>
            </div>
            <Button variant="ghost" size="sm" onClick={loadBooking} className="text-zinc-400"><RefreshCw className="w-4 h-4" /></Button>
          </div>
        ...
        <Tabs defaultValue="overview" className="space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <Card className="bg-zinc-900 border-zinc-800">
                ...
                <CardContent className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between"><span className="text-zinc-500">Dauer</span><span className="text-white">{dateInfo?.duration_days || tourInfo?.duration_days} Tage</span></div>
                  ...
                  {pickupInfo && (
                    <div className="flex justify-between"><span className="text-zinc-500">Zustieg</span><span className="text-white">{pickupInfo.city} ({pickupInfo.departure_time?.slice(0,5)} Uhr)</span></div>
                  )}
                </CardContent>
              </Card>
              ...
              <Card className="bg-zinc-900 border-zinc-800">
                ...
                <CardContent className="space-y-2 text-sm">
                  ...
                  {booking.pickup_surcharge > 0 && (
                    <div className="flex justify-between"><span className="text-zinc-500">Zustiegs-Aufpreis</span><span className="text-white">+{booking.pickup_surcharge.toFixed(2)}€</span></div>
                  )}
                  {booking.discount_amount && booking.discount_amount > 0 && (
                    <div className="flex justify-between"><span className="text-zinc-500">Rabatt ({booking.discount_code})</span><span className="text-emerald-400">-{booking.discount_amount.toFixed(2)}€</span></div>
                  )}
              ...
              <div className="grid grid-cols-3 gap-4">
                <Card className="bg-zinc-900 border-zinc-800">
                  <CardContent className="pt-4">
                    <div className="text-xs text-zinc-500 mb-1">Sollbetrag</div>
                    <div className="text-2xl font-bold text-white">{booking.total_price.toFixed(2)}€</div>
                  </CardContent>
                </Card>
                <Card className="bg-zinc-900 border-zinc-800">
                  <CardContent className="pt-4">
                    <div className="text-xs text-zinc-500 mb-1">Bezahlt</div>
                    <div className="text-2xl font-bold text-emerald-400">{totalPaid.toFixed(2)}€</div>
                  </CardContent>
                </Card>
                <Card className="bg-zinc-900 border-zinc-800">
                  <CardContent className="pt-4">
                    <div className="text-xs text-zinc-500 mb-1">Offen</div>
                    <div className={`text-2xl font-bold ${openAmount > 0 ? "text-red-400" : "text-emerald-400"}`}>{openAmount.toFixed(2)}€</div>
                  </CardContent>
                </Card>
              </div>
              ...
              <Card className="bg-zinc-900 border-zinc-800">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700" onClick={() => setPaymentModal(true)}>
                      <Plus className="w-3 h-3 mr-1" /> Zahlung erfassen
                    </Button>
                  </div>
                </CardHeader>
                <CardContent>
                  {payments.length === 0 ? (
                    <p className="text-zinc-500 text-sm py-4 text-center">Noch keine Zahlungseingänge</p>
                  ) : (
                    <Table>
                      <TableHeader>
                        <TableRow className="border-zinc-800">
                          ...
                          <TableHead className="text-zinc-400">Methode</TableHead>
                          <TableHead className="text-zinc-400">Referenz</TableHead>
          ...
          <TabsContent value="documents">
            <div className="space-y-4">
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Button size="sm" variant="outline" className="border-zinc-700 text-zinc-300" onClick={regenerateDocuments} disabled={processing === "regen"}>
                      {processing === "regen" ? <Loader2 className="w-3 h-3 animate-spin mr-1" /> : <RefreshCw className="w-3 h-3 mr-1" />}
                      Alle neu generieren
                    </Button>
                  </div>
                </CardHeader>
                      ...
                      <div key={doc} className="bg-zinc-800 rounded-lg p-4 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-zinc-500">PDF • Auto-generiert</div>
                          </div>
                        </div>
              ...
              <Card className="bg-zinc-900 border-zinc-800">
                ...
                <CardContent>
                  {docSends.length === 0 ? (
                    <p className="text-zinc-500 text-sm py-4 text-center">Noch keine Dokumente versendet</p>
                  ) : (
                    <Table>
                      <TableHeader>
                        <TableRow className="border-zinc-800">
                          <TableHead className="text-zinc-400">Zeitpunkt</TableHead>
                          <TableHead className="text-zinc-400">Dokument</TableHead>
                          <TableHead className="text-zinc-400">Empfänger</TableHead>
          ...
          <TabsContent value="communication">
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  {[
                    ...
                    <Button
                      key={action.type}
                      variant="outline"
                      className="border-zinc-700 text-zinc-300 justify-start h-auto py-3"
                      onClick={() => sendEmail(action.type)}
                      disabled={!!processing}
                    >
            ...
            <Card className="bg-zinc-900 border-zinc-800 mt-4">
              ...
              <CardContent>
                {docSends.length === 0 ? (
                  <p className="text-zinc-500 text-sm py-4 text-center">Keine E-Mails gesendet</p>
                ) : (
                  <div className="space-y-2">
                    {docSends.map((d) => (
                      <div key={d.id} className="bg-zinc-800 rounded-lg p-3 flex items-center justify-between">
                        <div>
                          <div className="text-sm text-white">{d.document_type}</div>
                          <div className="text-xs text-zinc-500">{format(new Date(d.created_at), "dd.MM.yyyy HH:mm", { locale: de })} → {d.recipient_email}</div>
                        </div>
          ...
          <TabsContent value="audit">
            <Card className="bg-zinc-900 border-zinc-800">
              ...
              <CardContent>
                {auditLog.length === 0 ? (
                  <p className="text-zinc-500 text-sm py-4 text-center">Keine Änderungen protokolliert</p>
                ) : (
                  <div className="space-y-2">
                    {auditLog.map((entry) => (
      ...
      <Dialog open={paymentModal} onOpenChange={setPaymentModal}>
        <DialogContent className="bg-zinc-900 border-zinc-800 text-white">
          ...
          <div className="space-y-4">
            <div>
              <Label className="text-zinc-400">Betrag (€)</Label>
              <Input
                type="number"
                step="0.01"
            ...
            <div>
              <Label className="text-zinc-400">Zahlungsart</Label>
            ...
            <div>
              <Label className="text-zinc-400">Referenz (z.B. Kontoauszug)</Label>
              <Input
                value={paymentForm.reference}
                onChange={(e) => setPaymentForm(f => ({ ...f, reference: e.target.value }))}
                className="bg-zinc-800 border-zinc-700 text-white"
                placeholder="z.B. Kontoauszug 15.03 / PayPal TX-123"
              />
            </div>
            <div>
              <Label className="text-zinc-400">Notiz (optional)</Label>
              <Textarea
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                onChange={(e) => setPaymentForm(f => ({ ...f, sendEmail: e.target.checked }))}
                className="rounded border-zinc-600"
              />
              <Label htmlFor="sendEmailCheckbox" className="text-zinc-400 text-sm cursor-pointer">Bestätigungs-E-Mail an Kunden senden</Label>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button className="bg-emerald-600 hover:bg-emerald-700" onClick={handlePaymentReceived} disabled={processing === "payment"}>
              {processing === "payment" ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <CheckCircle2 className="w-4 h-4 mr-1" />}
              Zahlung erfassen
            </Button>
          </DialogFooter>
        </DialogContent>
