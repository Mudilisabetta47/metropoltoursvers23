...
import { DollarSign, Download, ArrowLeft, Loader2, TrendingUp, AlertCircle, CheckCircle2 } from "lucide-react";
...
const AdminFinances = () => {
  ...
  const cancelledAmount = filteredBookings.filter(b => b.status === "cancelled").reduce((s: number, b: any) => s + Number(b.total_price), 0);
  const taxRate = 0.19;
  const netRevenue = totalPaid / (1 + taxRate);
  ...
  const exportCSV = () => {
    const rows = [
      ...
      ...filteredBookings.map((b: any) =>
        [b.booking_number, `${b.contact_first_name} ${b.contact_last_name}`, b.contact_email, b.total_price, b.status, b.payment_method || "", format(new Date(b.created_at), "dd.MM.yyyy"), b.paid_at ? format(new Date(b.paid_at), "dd.MM.yyyy") : ""].join(";")
      ),
    ].join("\n");
    ...
    a.href = url;
    a.download = `finanzen-export-${format(new Date(), "yyyy-MM-dd")}.csv`;
    a.click();
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold flex items-center gap-2"><DollarSign className="w-6 h-6 text-emerald-400" /> Finanzen</h1>
              <p className="text-sm text-zinc-500">Umsatz, offene Posten & Exporte</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" variant="outline" className="border-zinc-700 text-zinc-300" onClick={exportCSV}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          <Card className="bg-zinc-900 border-zinc-800"><CardContent className="pt-4"><div className="text-xs text-zinc-500 mb-1">Gesamtumsatz (brutto)</div><div className="text-2xl font-bold text-emerald-400">{totalRevenue.toFixed(2)}€</div></CardContent></Card>
          ...
          <Card className="bg-zinc-900 border-zinc-800"><CardContent className="pt-4"><div className="text-xs text-zinc-500 mb-1">Offene Posten</div><div className="text-2xl font-bold text-red-400">{openAmount.toFixed(2)}€</div></CardContent></Card>
          <Card className="bg-zinc-900 border-zinc-800"><CardContent className="pt-4"><div className="text-xs text-zinc-500 mb-1">Netto (19% MwSt)</div><div className="text-2xl font-bold text-white">{netRevenue.toFixed(2)}€</div></CardContent></Card>
        ...
        <Card className="bg-zinc-900 border-zinc-800 mb-6">
          ...
          <CardContent>
            {filteredPayments.length === 0 ? (
              <p className="text-zinc-500 text-sm py-4 text-center">Keine Zahlungen im gewählten Zeitraum</p>
            ) : (
              <Table>
                <TableHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-zinc-800">
                  ...
                  <TableHead className="text-zinc-400">Datum</TableHead>
                  <TableHead className="text-zinc-400">Bezahlt</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredBookings.slice(0, 100).map((b: any) => (
                  <TableRow key={b.id} className="border-zinc-800 cursor-pointer hover:bg-zinc-800" onClick={() => navigate(`/admin/booking/${b.id}`)}>
                    ...
                    <TableCell>
                      <Badge className={b.status === "confirmed" ? "bg-emerald-600" : b.status === "pending" ? "bg-amber-600" : "bg-red-600"}>
                        {b.status === "confirmed" ? "Bezahlt" : b.status === "pending" ? "Offen" : "Storno"}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-zinc-300">{format(new Date(b.created_at), "dd.MM.yy", { locale: de })}</TableCell>
                    <TableCell className="text-zinc-300">{b.paid_at ? format(new Date(b.paid_at), "dd.MM.yy", { locale: de }) : "–"}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
