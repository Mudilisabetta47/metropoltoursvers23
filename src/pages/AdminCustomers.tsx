...
const AdminCustomers = () => {
  ...
  const loadCustomers = async () => {
    ...
    if (bookings) {
      ...
      bookings.forEach((b: any) => {
        ...
        if (!grouped[key]) {
          grouped[key] = { email: b.contact_email, first_name: b.contact_first_name, last_name: b.contact_last_name, phone: b.contact_phone, booking_count: 0, total_spent: 0, last_booking: b.created_at, bookings: [] };
        }
    ...
    if (allNotes) {
      ...
      allNotes.forEach((n: any) => { if (!byEmail[n.customer_email]) byEmail[n.customer_email] = []; byEmail[n.customer_email].push(n); });
      setNotes(byEmail);
    }
    setIsLoading(false);
  };
  ...
  const addNote = async () => {
    ...
    await supabase.from("tour_customer_notes").insert({ customer_email: selectedCustomer.email, note: newNote.trim(), created_by: user?.id });
  ...
  const filtered = customers.filter(c => search === "" || c.email.toLowerCase().includes(search.toLowerCase()) || c.first_name.toLowerCase().includes(search.toLowerCase()) || c.last_name.toLowerCase().includes(search.toLowerCase()));

  return (
    <AdminLayout title="Kunden (CRM)" subtitle={`${customers.length} Kunden • ${customers.reduce((s, c) => s + c.total_spent, 0).toFixed(2)}€ Gesamtumsatz`}>
      {isLoading ? (
        <div className="flex justify-center py-12"><Loader2 className="w-8 h-8 animate-spin text-emerald-400" /></div>
      ) : (
        <div className="space-y-4">
          <Input placeholder="Suche nach Name oder E-Mail..." value={search} onChange={(e) => setSearch(e.target.value)} className="bg-zinc-800 border-zinc-700 text-white max-w-md" />

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card className="bg-zinc-900 border-zinc-800"><CardContent className="pt-4"><div className="text-2xl font-bold text-emerald-400">{customers.length}</div><div className="text-xs text-zinc-500">Kunden</div></CardContent></Card>
            ...
            <Card className="bg-zinc-900 border-zinc-800"><CardContent className="pt-4"><div className="text-2xl font-bold text-emerald-400">{customers.reduce((s, c) => s + c.total_spent, 0).toFixed(0)}€</div><div className="text-xs text-zinc-500">Umsatz</div></CardContent></Card>
            <Card className="bg-zinc-900 border-zinc-800"><CardContent className="pt-4"><div className="text-2xl font-bold text-white">{customers.length > 0 ? (customers.reduce((s, c) => s + c.total_spent, 0) / customers.length).toFixed(0) : 0}€</div><div className="text-xs text-zinc-500">Ø/Kunde</div></CardContent></Card>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-zinc-800">
                  ...
                  <TableHead className="text-zinc-400">Letzte</TableHead>
            ...
            <Card className="bg-zinc-900 border-zinc-800">
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Button size="sm" variant="outline" className="border-zinc-700 text-zinc-300" onClick={() => setNoteModal(true)}><Plus className="w-3 h-3 mr-1" /> Notiz</Button>
                </div>
              </CardHeader>
                ...
                {(notes[selectedCustomer.email.toLowerCase()] || []).length > 0 && (
                  <div className="space-y-2">
                    ...
                    {(notes[selectedCustomer.email.toLowerCase()] || []).map((n: any) => (
                      <div key={n.id} className="bg-zinc-800 rounded p-3"><div className="text-sm text-white">{n.note}</div><div className="text-xs text-zinc-500 mt-1">{format(new Date(n.created_at), "dd.MM.yy HH:mm", { locale: de })}</div></div>
                    ))}
                  </div>
                )}
                <div>
                  ...
                  <div className="space-y-2">
                    {selectedCustomer.bookings.map((b: any) => (
                      <div key={b.id} className="bg-zinc-800 rounded p-3 flex items-center justify-between cursor-pointer hover:bg-zinc-700" onClick={() => navigate(`/admin/booking/${b.id}`)}>
                        <div><span className="text-emerald-400 font-mono text-sm">{b.booking_number}</span><span className="text-zinc-400 text-sm ml-3">{format(new Date(b.created_at), "dd.MM.yy", { locale: de })}</span></div>
                        <div className="flex items-center gap-2"><span className="text-white font-medium">{Number(b.total_price).toFixed(2)}€</span><Badge className={b.status === "confirmed" ? "bg-emerald-600" : b.status === "pending" ? "bg-amber-600" : "bg-red-600"}>{b.status}</Badge></div>
                      </div>
                    ))}
                  </div>
                </div>
