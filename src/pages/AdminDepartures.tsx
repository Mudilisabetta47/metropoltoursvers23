...
import { Bus, Download, Loader2 } from "lucide-react";
...
const AdminDepartures = () => {
  ...
  const exportPassengerList = (dateEntry: any) => {
    ...
    dateBookings.forEach((b: any) => {
      ...
      if (details.length > 0) {
        details.forEach((p: any, i: number) => { passengers.push(`${i + 1};${p.firstName || p.first_name || b.contact_first_name};${p.lastName || p.last_name || b.contact_last_name};${b.booking_number};${b.contact_phone || ""}`); });
      } else {
        passengers.push(`1;${b.contact_first_name};${b.contact_last_name};${b.booking_number};${b.contact_phone || ""}`);
      }
    });
    const csv = [`Passagierliste – ${tour?.destination || "Reise"} – ${format(new Date(dateEntry.departure_date), "dd.MM.yyyy")}`, "", "Nr;Vorname;Nachname;Buchung;Telefon", ...passengers].join("\n");
    const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = `passagierliste-${tour?.destination || "reise"}-${dateEntry.departure_date}.csv`;
    a.click();
    toast({ title: "✅ Liste exportiert" });
  };
  ...
  return (
    <AdminLayout title="Abfahrten" subtitle={`${upcomingDates.length} kommende • ${pastDates.length} vergangene`}>
      {isLoading ? (
        <div className="flex justify-center py-12"><Loader2 className="w-8 h-8 animate-spin text-emerald-400" /></div>
      ) : (
        <div className="space-y-6">
          <h2 className="text-lg font-semibold text-white">Kommende Abfahrten</h2>
          <div className="space-y-3">
            {upcomingDates.length === 0 ? <p className="text-zinc-500 text-sm">Keine kommenden Abfahrten</p> : upcomingDates.map((d: any) => {
              const tour = getTour(d.tour_id);
              const pax = getBookingsForDate(d.id).reduce((s: number, b: any) => s + b.participants, 0);
              return (
                <Card key={d.id} className={`bg-zinc-900 border-zinc-800 cursor-pointer hover:border-emerald-600/50 ${selectedDate?.id === d.id ? "border-emerald-600" : ""}`} onClick={() => setSelectedDate(d)}>
                  <CardContent className="py-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className="bg-emerald-600/20 p-3 rounded-lg"><Bus className="w-5 h-5 text-emerald-400" /></div>
                        <div>
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right"><div className="text-sm"><span className="text-emerald-400 font-bold">{pax}</span><span className="text-zinc-500">/{d.total_seats}</span></div></div>
                        <Badge className={pax >= d.total_seats ? "bg-red-600" : pax > d.total_seats * 0.7 ? "bg-amber-600" : "bg-emerald-600"}>{Math.round((pax / d.total_seats) * 100)}%</Badge>
                        <Button size="sm" variant="outline" className="border-zinc-700 text-zinc-300" onClick={(e) => { e.stopPropagation(); exportPassengerList(d); }}><Download className="w-3 h-3 mr-1" /> Liste</Button>
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          {selectedDate && (
            <Card className="bg-zinc-900 border-zinc-800">
              <CardHeader><CardTitle className="text-sm text-zinc-400">Passagierliste – {getTour(selectedDate.tour_id)?.destination} – {format(new Date(selectedDate.departure_date), "dd.MM.yyyy")}</CardTitle></CardHeader>
              <CardContent>
                {getBookingsForDate(selectedDate.id).length === 0 ? <p className="text-zinc-500 text-sm py-4 text-center">Keine Buchungen</p> : (
                  <Table>
                    <TableHeader><TableRow className="border-zinc-800"><TableHead className="text-zinc-400">#</TableHead><TableHead className="text-zinc-400">Name</TableHead><TableHead className="text-zinc-400">Buchung</TableHead><TableHead className="text-zinc-400">Personen</TableHead><TableHead className="text-zinc-400">Telefon</TableHead><TableHead className="text-zinc-400">Status</TableHead></TableRow></TableHeader>
                    <TableBody>
          ...
          <h2 className="text-lg font-semibold text-zinc-400">Vergangene</h2>
            ...
            {pastDates.slice(0, 10).map((d: any) => {
              ...
              return (
                <div key={d.id} className="bg-zinc-900 border border-zinc-800 rounded-lg p-3 flex items-center justify-between opacity-60">
                  <div className="flex items-center gap-3"><Bus className="w-4 h-4 text-zinc-500" /><span className="text-zinc-300">{tour?.destination}</span><span className="text-zinc-500 text-sm">{format(new Date(d.departure_date), "dd.MM.yyyy")}</span></div>
