...
import { Shield, ArrowLeft, Loader2, Upload, FileText, CheckCircle2, Plus } from "lucide-react";
...
const AdminLegal = () => {
  ...
  const getTypeLabel = (t: string) => {
    switch (t) {
      ...
      case "widerruf": return "Widerrufsbelehrung";
      default: return t;
    }
  };
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold flex items-center gap-2"><Shield className="w-6 h-6 text-emerald-400" /> Rechtliches</h1>
              <p className="text-sm text-zinc-500">AGB, Datenschutz, Sicherungsschein – versioniert</p>
            </div>
          </div>
        ...
        <div className="space-y-6">
            ...
            <Card key={type} className="bg-zinc-900 border-zinc-800">
              <CardHeader>
                <CardTitle className="text-sm text-zinc-400 flex items-center gap-2">
                  <FileText className="w-4 h-4" /> {getTypeLabel(type)}
                </CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  {(versions as any[]).map((v: any) => (
                    <div key={v.id} className="bg-zinc-800 rounded p-3 flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-xs text-zinc-500">{v.version} • {format(new Date(v.created_at), "dd.MM.yyyy HH:mm", { locale: de })}</div>
                      </div>
                      <div className="flex items-center gap-2">
                        {v.is_current ? (
                          <Badge className="bg-emerald-600"><CheckCircle2 className="w-3 h-3 mr-1" /> Aktiv</Badge>
                        ) : (
                          <Badge className="bg-zinc-600">Archiv</Badge>
                        )}
                      </div>
                    </div>
                  ))}
          ...
          {Object.keys(grouped).length === 0 && (
            <p className="text-zinc-500 text-center py-8">Noch keine Dokumente angelegt</p>
          )}
        </div>
      </div>
      ...
      <Dialog open={addModal} onOpenChange={setAddModal}>
        <DialogContent className="bg-zinc-900 border-zinc-800 text-white">
          <DialogHeader><DialogTitle>Neues Dokument / Neue Version</DialogTitle></DialogHeader>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input value={form.version} onChange={(e) => setForm(f => ({ ...f, version: e.target.value }))} className="bg-zinc-800 border-zinc-700 text-white" placeholder="v1.0" />
              </div>
              <div>
                <Label className="text-zinc-400">Titel</Label>
                <Input value={form.title} onChange={(e) => setForm(f => ({ ...f, title: e.target.value }))} className="bg-zinc-800 border-zinc-700 text-white" placeholder="AGB Metropol Tours" />
              </div>
            </div>
            <div>
              <Label className="text-zinc-400">Inhalt</Label>
              <Textarea value={form.content} onChange={(e) => setForm(f => ({ ...f, content: e.target.value }))} className="bg-zinc-800 border-zinc-700 text-white" rows={8} placeholder="Inhalt hier einfügen..." />
            </div>
          </div>
          <DialogFooter>
