...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // AuthZ: require agent or admin role
    const { data: roles } = await supabase.from("user_roles").select("role").eq("user_id", userId);
    const isAuthorized = roles?.some((r: any) => r.role === "admin" || r.role === "agent" || r.role === "office");
    if (!isAuthorized) {
    ...
    if (!offerId || typeof offerId !== "string" ||
      ...
      return new Response(JSON.stringify({ error: "Invalid offerId" }), {
...
<body style="margin:0;padding:0;font-family:'Helvetica Neue',Arial,sans-serif;color:#1a1a1a;background:#fff;">
  <div style="max-width:680px;margin:0 auto;padding:40px 30px;">
    ...
    <div style="font-size:10px;color:#9ca3af;text-align:center;line-height:1.6;">
      ...
      E-Mail: kundenservice@app.metours.de · Tel: +49 511 80781106<br/>
